...
# commit status.
github_context = "aspect-8.5-gcc-hd"

# directory where the local checkout of the git repo sits
#repodir = "/ssd/auto/ssd-auto-aspect-8.5.0-hd/aspect"
...
def execute_test(sha1, name):
    ...
    cmd = './docker_with_timeout.sh 30m ' \
          ...
          '-e BUILDS=\'gcc\' ' \
          '-v "{2}:/source:ro" ' \
          '-v "$(pwd)/logs/{0}:/home/dealii/log" ' \
          'tjhei/aspect-tester-8.5.0'.format(sha1, name, repodir)
    print "cmd:",cmd
    try:
        answer = subprocess.check_output(cmd,
                                         shell=True,stderr=subprocess.STDOUT)
...
def is_allowed(username):
    trusted = ['tjhei', 'bangerth', 'jdannberg', 'gassmoeller', 'naliboff', 'anne-glerum', 'MFraters']
